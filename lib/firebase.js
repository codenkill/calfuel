...
// Initialize Stripe only if we're on the server side
let stripe;
...
export const addMeal = async (userId, mealData) => {
  try {
    ...
    // Ensure the meal has a unique ID
    const newMeal = {
      ...mealData,
      id: mealData.id || doc(collection(db, 'users', userId, 'meals')).id, // Generate a unique ID if not provided
    ...
    if (mealDoc.exists()) {
      ...
      const isDuplicate = currentMeals.some(meal => meal.id === newMeal.id);
      if (isDuplicate) {
        newMeal.id = doc(collection(db, 'users', userId, 'meals')).id; // Generate a new ID if duplicate
      }
...
export const getAllMeals = async (userId) => {
  try {
    ...
    const seenIds = new Set(); // Track seen meal IDs

    snapshot.forEach(doc => {
      const data = doc.data();
      if (data.meals && Array.isArray(data.meals)) {
        data.meals.forEach(meal => {
          // Skip meals with duplicate IDs
          if (meal.id && seenIds.has(meal.id)) {
            return;
          }
          // Add the meal ID to seen set
          if (meal.id) {
            seenIds.add(meal.id);
          }
          meals.push({
...
// Update a meal
export const updateMeal = async (userId, date, mealId, updatedMealData) => {
  try {
    const mealRef = doc(db, 'users', userId, 'meals', date);
    const mealDoc = await getDoc(mealRef);
    ...
    if (mealDoc.exists()) {
      ...
      const updatedMeals = currentMeals.map(meal =>
        meal.id === mealId
          ? { ...meal, ...updatedMealData, id: mealId }
          : meal
      );
...
export const getPaginatedMeals = async (userId, startDate, endDate, lastDoc = null, limitCount = 10) => {
  try {
    ...
    snapshot.forEach(doc => {
      ...
      if (data.meals && Array.isArray(data.meals)) {
        data.meals.forEach(meal => {
          // Generate a new ID if the meal doesn't have one
          const mealId = meal.id || doc(collection(db, 'users', userId, 'meals')).id;
          
          // Skip meals with duplicate IDs
          if (seenIds.has(mealId)) {
            return;
          }
          ...
          meals.push({
            ...meal,
            id: mealId, // Ensure the ID is set
            date: doc.id,
          });
        });
      }
    ...
    // Sort meals by date and timestamp
    meals.sort((a, b) => {
      if (a.date !== b.date) {
...
export const checkSubscriptionStatus = async (userId) => {
  try {
    ...
    // If user has a Stripe customer ID, verify subscription with Stripe
    if (userData.stripeCustomerId) {
      try {
        ...
        } else if (!isActive && userData.subscriptionStatus === 'active') {
